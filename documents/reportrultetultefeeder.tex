...
A fundamental part of this project was to select a friction model for which a good compensation could be made. As stated we chose to try a coulomb friction model and and model with added viscous part, the difference can be seen in figure \ref{fig:frictmodel}. After the results in \ref{fig:VLUF} we decided that since they were very similar we would focus on the coulomb+viscous friction model which had a higher potential to be close to the real friction.
Since we observed that the friction is different depending on the sign of the arm velocity in \ref{fig:fric} we added a constant offset to our regressor models, the coulomb+viscous friction regressors with and without offset can seen in equation \ref{eq:regressorb}  and \ref{eq:regressorbo} respectively. This leads to a better representation of the friction which can be seen in figure \ref{fig:VLUFoffset}. The discrepancy in the amplitude of the friction calculation is likely due to process changes, however \ref{fig:VLUFoffset} still shows the better representation of the calculated friction.
