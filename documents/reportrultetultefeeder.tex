...
\centering
		...
		\end{subfigure}
		\caption{Stepresonses for the pendulum and arm positions and velocities for the using the coloumb and viscouse friction model.}
		\label{fig:srvisc}
\end{figure}
\subsection{Friction estimation}
...
\subsection{}
Our initial thought was that this would be the reason for the offset we can see in \ref{fig:onoffcompensation} however after adding the offset we could still observe the same behaviour with only a small improvement, see \ref{fig:onoffcompoffset}. Instead we explored the option to add an integrative part in order to move the arm position so it would oscillate around the true zero position instead, the result of adding this integrative part was that the offset decreased and the result can be seen in \ref{fig:onoffcompintegrate}.
